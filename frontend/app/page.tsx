...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F4F0] dark:bg-[#14140F]">
      {/* ── Navbar ─────────────────────────────────────────────── */}
      <nav className="sticky top-0 z-50 bg-[#F5F4F0]/90 dark:bg-[#14140F]/90 backdrop-blur-md border-b border-[#DEDCD3] dark:border-[#35352C] transition-colors">
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-16">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2.5 font-heading font-semibold text-lg text-[#1B1B18] dark:text-[#F2F1EA] hover:no-underline group">
            <div className="w-8 h-8 bg-[#6B2737] flex items-center justify-center text-white text-sm font-bold shadow-sm rounded-lg transition-transform group-hover:scale-105">
              Q
            </div>
            <span className="tracking-tight font-medium text-lg">Questly</span>
          </Link>

          {/* Desktop Nav Links */}
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-[#5B5A52] dark:text-[#ABA99C]">
            <Link href="#features" className="hover:text-[#6B2737] dark:hover:text-[#B5677A] transition-colors py-1">
              Features
            </Link>
            <Link href="#how-it-works" className="hover:text-[#6B2737] dark:hover:text-[#B5677A] transition-colors py-1">
              How it works
            </Link>
            <Link href="#pricing" className="hover:text-[#6B2737] dark:hover:text-[#B5677A] transition-colors py-1">
              Pricing
            </Link>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {!authChecked && (
              <div className="w-20 h-8 bg-[#EDECE6] dark:bg-[#262620] animate-pulse rounded-lg" />
            )}

            {authChecked && authUser && (
              <div className="hidden sm:flex items-center gap-2">
                <div className="flex items-center gap-2 px-2.5 py-1 bg-white dark:bg-[#1C1C16] border border-[#DEDCD3] dark:border-[#35352C] rounded-lg">
                  <div className="w-6 h-6 bg-[#6B2737] text-white text-[11px] font-bold flex items-center justify-center rounded-md">
                    {authUser.initials}
                  </div>
                  <span className="text-xs font-medium text-[#1B1B18] dark:text-[#F2F1EA] truncate max-w-[120px]">
                    {authUser.name}
                  </span>
                </div>
                <Link
                  href="/dashboard"
                  className="text-xs font-semibold bg-[#6B2737] hover:bg-[#551F2C] text-white px-3.5 py-2 inline-flex items-center gap-1.5 transition-colors shadow-sm rounded-lg"
                >
            ...
            {authChecked && !authUser && (
              <div className="hidden sm:flex items-center gap-2">
                <Link
                  href="/login"
                  className="text-xs font-semibold text-[#5B5A52] hover:text-[#1B1B18] dark:text-[#ABA99C] dark:hover:text-[#F2F1EA] px-3 py-2 transition-colors rounded-lg"
                >
                  Sign in
                </Link>
                <Link
                  href="/signup"
                  className="text-xs font-semibold bg-[#6B2737] hover:bg-[#551F2C] text-white px-4 py-2 inline-flex items-center gap-1.5 transition-colors shadow-sm rounded-lg"
                >
                  Get started
            ...
            <button
              ...
              className="md:hidden p-2 border border-[#DEDCD3] dark:border-[#35352C] bg-white dark:bg-[#1C1C16] text-[#1B1B18] dark:text-[#F2F1EA] hover:bg-[#FAFAF8] dark:hover:bg-[#262620] transition-colors rounded-lg"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-[#DEDCD3] dark:border-[#35352C] bg-[#F5F4F0] dark:bg-[#14140F] px-6 py-5 space-y-4 animate-fade-in-up">
            <div className="flex flex-col space-y-3 font-medium text-sm text-[#5B5A52] dark:text-[#ABA99C]">
              <Link
                href="#features"
                onClick={() => setMobileMenuOpen(false)}
                className="hover:text-[#6B2737] dark:hover:text-[#B5677A] py-1 transition-colors"
              >
                Features
              </Link>
              <Link
            ...
            <div className="pt-3 border-t border-[#DEDCD3] dark:border-[#35352C] space-y-3">
              {authChecked && authUser && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 py-1">
                    <div className="w-7 h-7 bg-[#6B2737] text-white text-xs font-bold flex items-center justify-center rounded-lg">
                      {authUser.initials}
                    </div>
                    <div className="text-xs">
                      <div className="font-semibold text-[#1B1B18] dark:text-[#F2F1EA]">{authUser.name}</div>
                      <div className="text-[#8C8B82] truncate max-w-[200px]">{authUser.email}</div>
                    </div>
                  </div>
                  <Link
                    href="/dashboard"
                    onClick={() => setMobileMenuOpen(false)}
                    className="w-full text-xs font-semibold bg-[#6B2737] hover:bg-[#551F2C] text-white py-2.5 flex items-center justify-center gap-1.5 transition-colors rounded-lg"
                  >
                    <LayoutDashboard className="w-3.5 h-3.5" />
                    Go to Dashboard
                  </Link>
              ...
              {authChecked && !authUser && (
                <div className="flex flex-col gap-2 pt-1">
                  <Link
                    href="/login"
                    onClick={() => setMobileMenuOpen(false)}
                    className="w-full text-center text-xs font-semibold text-[#1B1B18] dark:text-[#F2F1EA] border border-[#DEDCD3] dark:border-[#35352C] bg-white dark:bg-[#1C1C16] py-2.5 transition-colors rounded-lg"
                  >
                    Sign in
                  </Link>
                  <Link
                    href="/signup"
                    onClick={() => setMobileMenuOpen(false)}
                    className="w-full text-center text-xs font-semibold bg-[#6B2737] hover:bg-[#551F2C] text-white py-2.5 flex items-center justify-center gap-1.5 transition-colors rounded-lg"
                  >
                    Get started free
                    <ChevronRight className="w-3.5 h-3.5" />
                  </Link>
      ...
      <section className="relative overflow-hidden border-b border-[#DEDCD3] dark:border-[#35352C] py-20 md:py-28">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3.5 mb-12">
            <Link
              href="/signup"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#6B2737] hover:bg-[#551F2C] text-white font-medium px-7 py-3.5 transition-all text-sm shadow-md hover:shadow-lg rounded-xl"
            >
              Start learning free
            ...
            <Link
              href="#how-it-works"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 border border-[#DEDCD3] dark:border-[#35352C] bg-white dark:bg-[#1C1C16] hover:bg-[#FAFAF8] dark:hover:bg-[#262620] text-[#1B1B18] dark:text-[#F2F1EA] font-medium px-7 py-3.5 transition-colors text-sm rounded-xl"
            >
              See how it works
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 mb-16 text-xs">
            ...
            {["Python", "React", "Machine Learning", "SQL", "Data Structures"].map((topic) => (
              <span
                key={topic}
                className="px-3 py-1 bg-white/80 dark:bg-[#1C1C16]/80 backdrop-blur-sm border border-[#DEDCD3] dark:border-[#35352C] text-[#5B5A52] dark:text-[#ABA99C] font-medium hover:border-[#6B2737] dark:hover:border-[#B5677A] transition-colors cursor-default rounded-full"
              >
                {topic}
              </span>
            ))}
          ...
          <div className="max-w-2xl mx-auto bg-white dark:bg-[#1C1C16] border border-[#DEDCD3] dark:border-[#35352C] p-4 shadow-sm grid grid-cols-3 divide-x divide-[#DEDCD3] dark:divide-[#35352C] text-center rounded-2xl">
            <div className="px-2">
